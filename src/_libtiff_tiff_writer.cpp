...
class LibtiffTiffWriter {
 public:
  LibtiffTiffWriter(fs::path filename, std::array<int, 3> imageSize,
                    ...
                    int quality = 100)
      : filename(std::move(filename).string()),
        imageSize(imageSize),
        mpp(mpp),
        tileSize(tileSize),
...
std::vector<std::byte> LibtiffTiffWriter::read2x2TileGroup(
  ...
  for (int i = 0; i < 2; ++i) {
    for (int j = 0; j < 2; ++j) {
      ...
      // Skip if this tile is out of bounds, this can happen
      // when the image dimensions are smaller than 2x2 in tileSize
...
void LibtiffTiffWriter::writeDownsampledResolutionPage(int level) {
  ...
  for (uint32_t tileY = 0; tileY < numTilesY; ++tileY) {
    for (uint32_t tileX = 0; tileX < numTilesX; ++tileX) {
      ...
      downsample2x2(groupBuffer, 2 * tileWidth, 2 * tileHeight,
