...
//
// Example program to extract a region from a WSI file using libdlup and
// VipsSlide backend.
//
// Usage:
//   get_region_vips filename x y width height [--level=0]
//   [--output=region.jpg] [--apply-color-profile]
...
int main(int argc, char* argv[]) {
  ...
  CLI::App app{"Extract region from a whole slide image using VIPS backend"};
  ...
  try {
    dlup::backends::VipsSlide slide(filename, false, false,
                                    apply_color_profile);

    std::cout << "=== VIPS Backend ===\n";
