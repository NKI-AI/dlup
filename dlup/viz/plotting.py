...
def draw_used_tiles(
    input_file_path: pathlib.Path, tile_size: int, thumbnail: PIL.Image.Image, coords: List, original_mpp: float
) -> PIL.Image.Image:
    """
    ...
    thumbnail_with_used_tiles = thumbnail.copy()
    ...
    converted_tile_size = (
        int(tile_size / (scaled_view_size[1] / thumbnail.size[1])),
        int(tile_size / (scaled_view_size[0] / thumbnail.size[0])),
    )
    coordinate_scales = ((scaled_view_size[1] / thumbnail.size[1]), (scaled_view_size[0] / thumbnail.size[0]))
