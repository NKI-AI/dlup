...
def _analyse_validity(shapely_polygons: list[MultiPolygon | Polygon]) -> list[Polygon]:
    """
    Analyze polygons returned by the DFS algorithm and return only valid polygons.

    Parameters
    -----------
    shapely_polygons: list[MultiPolygons | Polygon]
        The list of polygons generated by the DFS algorithm.

    Returns
    -------
    valid_polygons: list[Polygons]
    ...
    for polygon in shapely_polygons:
        ...
        if polygon.geom_type == "MultiPolygon":
            valid_polygons.extend([geom for geom in polygon.geoms if geom.is_valid and geom.area > 0 and not geom.is_empty])
