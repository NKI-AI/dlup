...
class ImageWriter(abc.ABC):
    ...
    def __init__(
        ...
        if quality is not None and compression in [TiffCompression.ZSTD, TiffCompression.LZW, TiffCompression.NONE]:
            raise ValueError(f"Quality is not supported for the {compression} compression type.")
...
class TifffileImageWriter(ImageWriter):
    ...
    def from_tiles_iterator(self, iterator: Iterator[npt.NDArray[np.int_]]) -> None:
        ...
        n_subresolutions = 0
        if self._pyramid:
            # If the image is smaller than the configured tile size, the log2 term becomes negative.
            # We still want to write a valid TIFF (just without additional pyramid levels).
            n_subresolutions = int(np.ceil(np.log2(np.asarray(native_size) / np.asarray(self._tile_size))).min())
            n_subresolutions = max(0, n_subresolutions)
    ...
    def _write_page(
        ...
        **options: Any,
        ...
        if self._quality is not None:
            options["compressionargs"] = {"quality": self._quality}
...
def _tile_iterator_from_page(
    ...
    for coordinates in grid:
        ...
        fim_tile = get_tile(page, (coordinates[1], coordinates[0]), (size[1], size[0]))
        output = fim_tile.downsample(2).to_numpy()
        yield output
