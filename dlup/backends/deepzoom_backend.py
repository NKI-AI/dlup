...
class AbstractDeepZoomSlide(AbstractSlideBackend):
    ...
    @property
    def mode(self) -> str:
        """Returns the mode of the deep zoom tiles. This is determined by the mode of the deepzoom tile at level 0.
        This is an image of size 1x1 that should exist for every deep zoom image.
        NOTE: When generating deepzoom pyramid with VIPS, this could be CYMK and differ from the original slide
        """
        if not hasattr(self, "_mode"):
            _region = self._open_deepzoom_tile(self._fetch_deepzoom_tile_files(0, [(0, 0)])[0])
            self._mode: str = _region.interpretation
        return self._mode
    ...
    def slide_bounds(self) -> tuple[tuple[int, int], tuple[int, int]]:
        ...
        # NOTE: If MRXS file is generated with a rotation (e.g. --angle d90), the metadata does not get updated.
        # In that case, x and width should be switched with y and height respectively
    ...
    @abc.abstractmethod
    def _fetch_deepzoom_tile_files(self, level: int, indices: list[tuple[int, int]]) -> TileResponseTypes:
    ...
    @abc.abstractmethod
    def _open_deepzoom_tile(
        self,
        filename: Any,
    ) -> pyvips.Image:
        """Open a single deepzoom tile. This function should return a pyvips.Image object."""
        ...
    ...
    def read_region(self, coordinates: tuple[Any, ...], level: int, size: tuple[int, int]) -> pyvips.Image:
        ...
        tile_files = self._fetch_deepzoom_tile_files(level_dz, indices)
        ...
        # faster, but does not work for unregular grids of images (the one we have).
        ...
        for (row, col), tile_file in zip(indices, tile_files):
            _region_tile = self._open_deepzoom_tile(tile_file)
            ...
            if crop_width > 0 and crop_height > 0:
                _cropped_region_tile = _region_tile.crop(crop_start_x, crop_start_y, crop_width, crop_height)
...
class DeepZoomSlide(AbstractDeepZoomSlide):
    ...
    def _fetch_deepzoom_tile_files(self, level: int, indices: list[tuple[int, int]]) -> list[str]:
        """Retrieve paths for tile indices of deepzoom level. These tiles will be opened with pyvips in
        `self._open_deepzoom_tile` and stitched together in `read_region`.
        """
    ...
    def _open_deepzoom_tile(self, filename: Path) -> pyvips.Image:
