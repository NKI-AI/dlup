...
class AbstractSlideBackend(abc.ABC):
    ...
    def get_thumbnail(self, size: int | tuple[int, int]) -> fim.Image:
        """
        Return a fim.Image as an RGB image with the thumbnail with maximum size given by size.
        ...
        # Get dimensions from fim.Image
        thumb_dims = thumbnail.dimensions
        thumb_width, thumb_height, thumb_channels = thumb_dims

        # Calculate target size while preserving aspect ratio
        scale_x = size[0] / thumb_width
        scale_y = size[1] / thumb_height
        scale_factor = min(scale_x, scale_y)

        target_width = int(thumb_width * scale_factor)
        target_height = int(thumb_height * scale_factor)

        # Resize using fim - use NEAREST for safety with masks/colormaps
        thumbnail = thumbnail.resize(target_width, target_height, kernel=fim.KernelType.NEAREST)

        # If 4 channels (RGBA), flatten to RGB
        if thumb_channels == 4:
            # Convert to numpy, remove alpha, convert back
    ...
    def read_region(self, coordinates: tuple[int, int], level: int, size: tuple[int, int]) -> fim.Image:
        ...
        -------
        fim.Image
            The requested region as a fim image.
        """
