...
# from dlup.experimental_backends.deepzoom_backend import DeepZoomSlide
# from dlup.data.dataset import TiledWsiDataset
# from dlup.tiling import TilingMode
...
# class V7RemoteTiledImage(RemoteSlideBackend, DeepZoomSlide):
#     METADATA_CACHE = 128  # Assuming a constant for cache size
...
#     def _set_metadata(self) -> None:
#         self._metadata = self._fetch_v7_metadata()

#     @functools.lru_cache(maxsize=METADATA_CACHE)
#     def _fetch_properties(self) -> dict[str, Any]:
#         _properties = {"mpp_x": self._metadata["mpp"], "mpp_y": self._metadata["mpp"]}
#         return _properties

#     @functools.lru_cache(maxsize=METADATA_CACHE)
#     def _fetch_dz_properties(self) -> dict[str, Any]:
#         tile_size = (self._metadata["levels"]["0"]["tile_width"], self._metadata["levels"]["0"]["tile_height"])
#         size_dict = {
#             "width": self._metadata["full_size"][0],
#             "height": self._metadata["full_size"][1],
#         }
#         _dz_properties = {"image": {"size": size_dict, "format": "jpg", "tile_size": tile_size, "overlap": 0}}
#         return _dz_properties

#     @functools.lru_cache(maxsize=METADATA_CACHE)
#     def _fetch_v7_metadata(self) -> dict[str, Any]:
#         file_stream = io.BytesIO()
#         try:
#             self._s3_client.download_fileobj(self._bucket, f"{self._bucket_prefix}metadata.json", file_stream)
#             file_stream.seek(0)
...
#     def read_tile_from_remote(self, level: int, row: int, col: int) -> PIL.Image.Image:
#         if level < 0 or level >= len(self._downsamples):
#             raise ValueError(f"Level {level} is out of bounds. The number of levels is {len(self._downsamples)}")
...
#         except:
#             # TODO: The out of bounds read should return an empty tile
#             image = PIL.Image.new(self.mode, self._tile_size, (255, 255, 255))
#             return image

#     def retrieve_deepzoom_tiles(self, level: int, indices: list[tuple[int, int]]) -> list[str | io.BytesIO]:
...
#         """
#         return [self.read_tile_from_remote(level, row=row, col=col) for col, row in indices]
