...
#ifndef AIFO_DLUP_INCLUDE_DLUP_TIFF_WRITER_H_
#define AIFO_DLUP_INCLUDE_DLUP_TIFF_WRITER_H_
...
#include <utility>
#include <vector>
#include "dlup/constants.h"
#include "dlup/image.h"
#include "dlup/tiff/exceptions.h"
...
class LibtiffTiffWriter {
 ...
 private:
  ...
  std::vector<std::byte> read2x2TileGroup(TIFF* readTif, uint32_t row,
                                          uint32_t col, uint32_t prevWidth,
                                          uint32_t prevHeight);
  void setupReadTIFF(TIFF* readTif);
};

LibtiffTiffWriter::~LibtiffTiffWriter() {
  finalize();
}
...
void LibtiffTiffWriter::writeTile(
  ...
  if (row < 0 || row >= imageSize[0] || col < 0 || col >= imageSize[1]) {
    ...
    throw TiffWriteException(
        "tile coordinates out of bounds for row " + std::to_string(row) +
        ", col " + std::to_string(col) + ". Image Size is " +
...
void LibtiffTiffWriter::validateInputs() const {
  ...
  if ((tileSize[0] & (tileSize[0] - 1)) != 0 ||
      (tileSize[1] & (tileSize[1] - 1)) != 0) {
...
void LibtiffTiffWriter::finalize() {
  if (tif) {
    ...
    if (TIFFCurrentDirectory(tif.get()) <
        TIFFNumberOfDirectories(tif.get()) - 1) {
...
void LibtiffTiffWriter::setupReadTIFF(TIFF* readTif) {
  auto set_field = [readTif](uint32_t tag, auto... value) {
    if (TIFFSetField(readTif, tag, value...) != 1) {
      throw TiffSetupException("failed to set TIFF field for reading: " +
                               std::to_string(tag));
    }
  };
...
void LibtiffTiffWriter::setupTIFFDirectory(int level) {
  auto set_field = [this](uint32_t tag, auto... value) {
    if (TIFFSetField(tif.get(), tag, value...) != 1) {
      throw TiffSetupException("failed to set TIFF field: " +
                               std::to_string(tag));
    }
  };
  ...
  // TODO(jonasteuwen): This needs to be configurable
  ...
  std::string software_tag = "dlup " + std::string(DLUP_VERSION) +
                             " (libtiff " + std::to_string(TIFFLIB_VERSION) +
                             ")";
...
std::vector<std::byte> LibtiffTiffWriter::read2x2TileGroup(
    TIFF* readTif, uint32_t row, uint32_t col, uint32_t prevWidth,
    uint32_t prevHeight) {
  auto [tileWidth, tileHeight] = tileSize;
  int channels = imageSize[2];
  ...
  std::vector<std::byte> groupBuffer(
      fullGroupWidth * fullGroupHeight * channels, std::byte(0));
  ...
  for (int i = 0; i < 2; ++i) {
    for (int j = 0; j < 2; ++j) {
      ...
      // Skip if this tile is out of bounds, this can happen when
      // the image dimensions are smaller than 2x2 in tileSize
      ...
      if (TIFFReadTile(readTif, tileBuf.data(), tileCol, tileRow, 0, 0) < 0) {
        throw TiffReadException("failed to read tile at row " +
                                std::to_string(tileRow) + ", col " +
      ...
      uint32_t copyHeight =
          std::min<uint32_t>(tileHeight, prevHeight - tileRow);
      for (uint32_t y = 0; y < copyHeight; ++y) {
        for (uint32_t x = 0; x < copyWidth; ++x) {
          for (int c = 0; c < channels; ++c) {
            size_t groupIndex =
                ((i * tileHeight + y) * fullGroupWidth + (j * tileWidth + x)) *
                    channels +
                c;
            size_t tileIndex = (y * tileWidth + x) * channels + c;
            groupBuffer[groupIndex] =
                static_cast<std::byte>(tileBuf[tileIndex]);
          }
        }
      }
    }
...
void LibtiffTiffWriter::writeDownsampledResolutionPage(int level) {
  if (level <= 0 || level >= numLevels) {
    throw std::invalid_argument(
        "Invalid level for downsampled resolution page");
  }
  ...
  if (!TIFFSetDirectory(readTif.get(), level - 1)) {
    throw TiffReadException("failed to set directory to level " +
  ...
  if (!TIFFWriteDirectory(tif.get())) {
    throw TiffWriteException(
        "failed to create new directory for downsampled image");
  }
  ...
  for (uint32_t tileY = 0; tileY < numTilesY; ++tileY) {
    for (uint32_t tileX = 0; tileX < numTilesX; ++tileX) {
      ...
      std::vector<std::byte> groupBuffer =
          read2x2TileGroup(readTif.get(), row, col, prevWidth, prevHeight);
      std::vector<std::byte> downsampledBuffer(tileHeight * tileWidth *
                                               channels);

      image_utils::downsample2x2(groupBuffer, 2 * tileWidth, 2 * tileHeight,
                                 downsampledBuffer, tileWidth, tileHeight,
                                 channels);

      if (TIFFWriteTile(tif.get(),
                        reinterpret_cast<uint8_t*>(downsampledBuffer.data()),
                        tileX * tileWidth, tileY * tileHeight, 0, 0) < 0) {
        throw TiffWriteException("failed to write downsampled tile at level " +
                                 std::to_string(level) + ", row " +
                                 std::to_string(tileY) + ", col " +
                                 std::to_string(tileX));
      }
    }
  }
...
#endif  // AIFO_DLUP_INCLUDE_DLUP_TIFF_WRITER_H_
