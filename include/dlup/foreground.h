...
class Foreground {
 public:
   ...
   * tile in the `SlideImage`, calculating their average coverage, and keeping
   * only the tiles that meet the specified threshold.
   *
   ...
   * @param mpp The microns-per-pixel scaling to use when reading the mask
   * regions.
   * @param threshold (Optional) The coverage threshold. Only tiles with average
   * coverage greater than this value will be kept. If no value is provided or
   * the value is negative, all tiles are kept. Value is in the range [0, 1].
  ...
  /**
   * @brief Filters a grid of tiles based on the average coverage of regions in
   * an AbstractSlideBackend.
   *
   * This method filters the grid by reading regions corresponding to each grid
   * tile in the `AbstractSlideBackend`, calculating their average coverage, and
   * keeping only the tiles that meet the specified threshold.
   *
   * @param grid The grid of tiles to filter.
...
ForegroundResult<T> Foreground<T>::FilterGrid(
  ...
  aifocore::Result<double> level_downsample = backend.GetLevelDownsample(level);
  ...
  for (size_t idx = 0; idx < grid.Length(); ++idx) {
    ...
    if (!region.ok()) {
      std::cerr << "Failed to read region for tile " << idx << ": "
                << region.status().message() << std::endl;
    }
